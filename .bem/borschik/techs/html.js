...
var BORSCHIK = require('borschik'),
    ...
    CSSBASE = BORSCHIK.require('./techs/css-base');
...
exports.Tech = INHERIT(CSSBASE.Tech, {
    ...
    File: exports.File = INHERIT(CSSBASE.File, {
        ...
        parseInclude: function(content) {
            ...
            var texts = text
                .replace(allIncRe, function(_, includeHref, includeSrc) {
                    if (includeHref && _this.isFreezableUrl(includeHref)) {
                        includes.push({
                            url: _this.pathTo(includeHref),
                            type: 'href'
                        });

                    } else if (includeSrc && _this.isFreezableUrl(includeSrc)) {
                        includes.push({
        ...
        isFreezableUrl: function(url) {
            return !/^(https?:)?\/\//.test(url) && (this.__base(url) || additionalFreezeExtsRe.test(url));
        }
